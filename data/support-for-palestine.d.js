/**
 * Dataset of different instances of global demonstration of support for the Palestinian cause, since the start of the latest imperial assault of the peoples of Gaza.
 * @typedef {Object} SupportForPalestine
 * @property {String} date - Format: d/m/y
 * @property {String} location - The name of the place (usually a city); here, name of the country has been written first for easier sorting.
 * @property {Url} video - URL to the video showing support
 * @property {ENotation} figure - Or, exponent. Example: since 5000 = 5*10^3, use '3'
 * @property {Url} [source] - The source of the `figure`. Can be empty if the same as `video`
 */
